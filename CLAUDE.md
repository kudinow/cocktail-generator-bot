# Правила проекта — Cocktail Telegram Bot

## ⚠️ ВАЖНО: Переход с TheCocktailDB на Inshaker

**Дата изменения:** 2026-02-14
**Обновление:** 2026-02-20 — Добавлена система управления парсингом через таблицы; запущен SEO-сайт alcoberry.ru; сетка ингредиентов переведена на 2 кнопки в ряд (длинные названия влезают полностью)

Проект перешёл с TheCocktailDB API на локальную базу данных Inshaker.

### Что изменилось

- ❌ **Удалено:** TheCocktailDB API (`cocktailService.ts`)
- ✅ **Добавлено:** Локальная база из 429 рецептов Inshaker ([data/inshaker_recipes.json](data/inshaker_recipes.json))
- ✅ **Добавлено:** Система управления парсингом через таблицы (см. [Система таблиц для парсинга](#система-таблиц-для-парсинга))
- 🔧 **Архивировано:** Старый код сохранён в директории [archive/](archive/)

### Преимущества нового подхода

- ✅ Все рецепты на **русском языке**
- ✅ Измерения в **миллилитрах** (мл) вместо унций (oz)
- ✅ Нет зависимости от внешнего API
- ✅ Быстрый доступ к данным (без сетевых запросов)
- ✅ Удобное управление парсингом через JSON-таблицы

### Текущий статус

- ✅ **Работает:** Поиск коктейлей по ингредиентам ([searchHandler.ts](src/handlers/searchHandler.ts))
- ✅ **Работает:** Поиск коктейлей по названию ([searchHandler.ts](src/handlers/searchHandler.ts))
- ✅ **Работает:** Поиск коктейлей по одному ингредиенту ([searchHandler.ts](src/handlers/searchHandler.ts))
- ✅ **Работает:** Пагинация результатов поиска (кнопка "Показать еще 10")
- ✅ **Работает:** Управление ингредиентами ([ingredientsHandler.ts](src/handlers/ingredientsHandler.ts))
- 📊 **База данных:** 429 рецептов (429 алкогольных, 0 безалкогольных)
- 🌐 **Сайт:** https://alcoberry.ru (824 страницы, Яндекс Object Storage, HTTPS)

### Как вернуть TheCocktailDB API

См. инструкции в [archive/README.md](archive/README.md)

---

## Архитектура

- **Точка входа**: `src/bot.ts` — создаёт экземпляры сервисов и регистрирует обработчики
- **Обработчики** (`src/handlers/`): каждый файл экспортирует функцию-регистратор (`handleX`) и отдельные функции-действия (`sendX`) для прямого вызова
- **Сервисы** (`src/services/`): бизнес-логика и работа с данными
  - `storageService.ts` — хранение данных пользователей
  - `translationService.ts` — перевод через OpenRouter API (GPT-3.5-turbo)
  - ~~`cocktailService.ts`~~ — **АРХИВИРОВАНО** (см. `archive/services/`)
- **Утилиты** (`src/utils/helpers.ts`): форматирование, перевод, вспомогательные функции
- **Типы** (`src/types/index.ts`): все TypeScript-интерфейсы
- **Данные** (`data/`):
  - `users.json` — данные пользователей (ингредиенты, активность)
  - `inshaker_recipes.json` — база из 429 рецептов коктейлей (Inshaker)
  - `parsed_ids.json` — ID успешно спарсенных рецептов
  - `failed_ids.json` — ID с ошибками парсинга (чёрный список)
  - `to_parse.json` — очередь ID на парсинг
- **Скрипты** (`scripts/`):
  - `parseInshaker.ts` — парсинг рецептов с Inshaker.com (см. [Система таблиц для парсинга](#система-таблиц-для-парсинга))

### Экспортируемые функции-действия
Каждый обработчик экспортирует функции, которые можно вызвать напрямую из любого места:
- `sendStart(bot, chatId, userId, storage, username?)` — из `startHandler.ts`
- `sendAddIngredient(bot, chatId, userId, storage)` — из `ingredientsHandler.ts`
- `sendMyIngredients(bot, chatId, userId, storage)` — из `ingredientsHandler.ts`
- `sendFindCocktails(bot, chatId, userId, storage, inshakerService, offset?)` — из `searchHandler.ts` (с пагинацией)
- `sendSearchByName(bot, chatId, userId)` — из `searchHandler.ts`
- `sendSearchByIngredient(bot, chatId, userId)` — из `searchHandler.ts` (поиск по одному ингредиенту)

## Критические правила Telegram Bot API

### Бот НЕ получает свои собственные сообщения
`bot.sendMessage(chatId, '/command')` из callback-обработчика **НЕ вызовет** `onText` обработчик.
Бот через polling получает только сообщения от пользователей, а не от себя.

**Неправильно:**
```typescript
bot.on('callback_query', async (query) => {
  await bot.sendMessage(chatId, '/start'); // НЕ вызовет onText(/\/start/)
});
```

**Правильно:** вызывать экспортированную функцию-действие напрямую:
```typescript
import { sendStart } from './startHandler';

bot.on('callback_query', async (query) => {
  await sendStart(bot, chatId, userId, storage);
});
```

### Каждый callback-обработчик фильтрует только свои callback_data
Все файлы регистрируют `bot.on('callback_query')`, и Telegram рассылает ВСЕ callback'и всем слушателям. Поэтому каждый обработчик ОБЯЗАН в начале проверять, относится ли `data` к нему, и выходить через `return` для чужих.

```typescript
// searchHandler — обрабатывает только свои
if (data !== 'find_cocktails' && !data.startsWith('show_recipe_')) return;

// ingredientsHandler — обрабатывает только свои
const ownCallbacks = ['add_ingredient', 'my_ingredients', ...];
if (!ownCallbacks.includes(data) && !data.startsWith('add_ing_') && !data.startsWith('remove_ing_')) return;
```

### Всегда вызывать answerCallbackQuery
Каждый обработчик `callback_query` ОБЯЗАН вызвать `bot.answerCallbackQuery(query.id)`, иначе в Telegram будет бесконечный спиннер загрузки.

### Лимит callback_data — 64 байта
`callback_data` в inline-кнопках ограничен 64 байтами. Для длинных значений использовать индексы или короткие ID, а не полные строки.

### Ошибка "message is not modified"
`editMessageText` бросает 400, если новый текст и клавиатура идентичны текущим (например, двойной клик). Оборачивать в try/catch и игнорировать эту ошибку:
```typescript
try {
  await bot.editMessageText(...);
} catch (editError: any) {
  if (!editError?.message?.includes('message is not modified')) throw editError;
}
```

### Telegram Markdown
При использовании `parse_mode: 'Markdown'` экранировать спецсимволы: `_`, `*`, `[`, `]`, `(`, `)` и др. Для команд в тексте: `/add\\_ingredient`.

## UX-паттерны

### Toggle-выбор ингредиентов
- Экран `/add_ingredient` показывает сетку популярных ингредиентов **по 2 кнопки в ряд** (`index % 2`) — длинные названия не обрезаются
- Уже выбранные помечаются `✅` в тексте кнопки
- Повторное нажатие убирает ингредиент (toggle)
- Клавиатура обновляется in-place через `editMessageText`
- Функции `buildIngredientKeyboard()` и `buildIngredientMessage()` строят UI с учётом текущего выбора

## Локализация

### Весь пользовательский интерфейс — на русском языке
- Все сообщения бота, кнопки, подсказки, ошибки — только на русском
- Пользователь может вводить названия коктейлей на русском языке
- Все рецепты отображаются полностью на русском (название, категория, инструкции, тип бокала)

### Двухуровневая модель данных
- **Внутренний слой** (storage, API): хранит и передаёт данные на английском (TheCocktailDB работает только с английскими названиями)
- **Отображение**: всегда переводить на русский через `TranslationService` или `translateToRussian()` из `helpers.ts`

### Система перевода (два уровня)

#### 1. Словарный перевод (helpers.ts) — для ингредиентов
- Словарь `ingredientMap` в `src/utils/helpers.ts`
- Быстрый перевод популярных ингредиентов без API-запросов
- `translateToEnglish(ru)` — для сохранения пользовательского ввода
- `translateToRussian(en)` — для отображения данных из API/storage
- Если перевода нет в словаре — возвращается оригинальная строка

#### 2. AI-перевод (TranslationService) — для названий и рецептов
- Использует OpenRouter API с моделью GPT-3.5-turbo
- **Функции:**
  - `translateToEnglish(text)` — перевод названия коктейля с русского на английский перед поиском
  - `translateToRussian(text)` — перевод названия, категории, инструкций, типа бокала на русский
  - `translateBatchToRussian(texts[])` — пакетный перевод для оптимизации
- **Кэширование:** все переводы кэшируются в памяти (Map) для ускорения повторных запросов
- **Конфигурация:** токен `OPENROUTER_API_KEY` в `.env` или fallback значение в `config.ts`
- **Обработка ошибок:** при ошибке API возвращается оригинальный текст без перевода

### Workflow перевода при поиске по названию
1. Пользователь вводит "мохито" → `translationService.translateToEnglish("мохито")` → "Mojito"
2. Поиск в TheCocktailDB API с английским названием
3. Получение рецепта на английском
4. Перевод всех полей рецепта через `translationService.translateToRussian()`
5. Отображение полностью русского рецепта пользователю

## Обработка ошибок

- Все обработчики обёрнуты в try/catch
- В catch для callback: всегда вызывать `answerCallbackQuery` с сообщением об ошибке (в своём try/catch, чтобы не потерять основную ошибку)
- Ошибки логировать через `console.error` с контекстом: `[CALLBACK_ERROR] userId=... data="..."`

## Логирование

- Каждое действие логируется с тегом: `[START]`, `[ADD_INGREDIENT]`, `[MY_INGREDIENTS]`, `[FIND_COCKTAILS]`
- Callback'и: `[INGREDIENT_CB]`, `[SEARCH_CALLBACK]`
- Ошибки: `[CALLBACK_ERROR]`
- Формат: `[TAG] userId=... chatId=... key=value`

## Данные и хранилище

- Данные пользователей хранятся в `data/users.json` (JSON-файл)
- `StorageService` загружает данные при старте и сохраняет при каждом изменении
- Ингредиенты хранятся на английском; сравнение регистронезависимое (`.toLowerCase()`)
- `userStates` (Map) и `userCocktails` (Map) живут в памяти — теряются при перезапуске

## Стиль кода

- TypeScript strict mode
- Async/await для всех операций с Bot API
- Проверять `userId` и `chatId` в начале каждого обработчика
- Конфигурация через `.env` и `src/config/config.ts`
- Не хардкодить лимиты — использовать `config.maxIngredientsPerUser`, `config.maxCocktailsToShow`

## Конфигурация и переменные окружения

Файл `.env` должен содержать:
```
TELEGRAM_BOT_TOKEN=your_telegram_bot_token
OPENROUTER_API_KEY=your_openrouter_api_key
COCKTAIL_API_URL=https://www.thecocktaildb.com/api/json/v1/1  # Опционально
```

**Важно:**
- `TELEGRAM_BOT_TOKEN` — обязателен, иначе приложение не запустится
- `OPENROUTER_API_KEY` — обязателен для работы перевода названий коктейлей и рецептов
- Токены хранятся **только в `.env`** — никогда не коммитьте их в Git
- Используйте `.env.example` как шаблон для настройки окружения

## Запуск

- `npm run dev` — разработка с hot-reload (nodemon + ts-node, `data/` игнорируется)
- `npm run build && npm start` — production сборка
- `npm run parse:inshaker` — парсинг новых рецептов с Inshaker (см. [Система таблиц для парсинга](#система-таблиц-для-парсинга))
- Требуется `TELEGRAM_BOT_TOKEN` в `.env`
- **Важно**: nodemon настроен с `--ignore data/`, иначе запись в `users.json` вызывает бесконечный цикл перезапусков

## Система таблиц для парсинга

**Обновление:** 2026-02-20 — Внедрена система управления парсингом через JSON-таблицы

Скрипт `scripts/parseInshaker.ts` использует три JSON-таблицы для управления процессом парсинга рецептов с ru.inshaker.com:

### Три таблицы парсинга

**1. `data/to_parse.json` — Очередь на парсинг**
- Сюда вы добавляете ID или ссылки на коктейли, которые нужно спарсить
- Поддерживает форматы:
  - Одиночный ID: `724`
  - Ссылка: `"https://ru.inshaker.com/cocktails/724"`
  - **Диапазон: `"160-165"`** (расширяется в 160, 161, 162, 163, 164, 165)
- Автоматически очищается после обработки

**2. `data/parsed_ids.json` — Успешно спарсенные**
- Содержит ID всех коктейлей, которые успешно добавлены в базу
- Автоматически обновляется при успешном парсинге
- ID из этой таблицы пропускаются при повторном парсинге

**3. `data/failed_ids.json` — Чёрный список**
- Содержит ID, которые не удалось спарсить (404, ошибки)
- Хранит метаданные: код ошибки, текст ошибки, дату попытки
- ID из этой таблицы **никогда не парсятся** повторно (защита от лишних запросов)

### Логика работы

```
Для каждого ID из to_parse.json:
  ├─ Есть в parsed_ids.json? → Пропускаем (уже спарсено)
  ├─ Есть в failed_ids.json? → Пропускаем (в чёрном списке)
  └─ Нет нигде → ПАРСИМ
      ├─ Успех → добавляем в parsed_ids.json + базу рецептов
      └─ Ошибка (404 и т.д.) → добавляем в failed_ids.json

После обработки: to_parse.json автоматически очищается
```

### Пошаговая инструкция

1. **Найдите ID коктейля** на сайте ru.inshaker.com
   - Например: `https://ru.inshaker.com/cocktails/150` → ID = `150`

2. **Добавьте ID в очередь** `data/to_parse.json`:
   ```json
   [
     "160-165",
     200,
     "https://ru.inshaker.com/cocktails/300"
   ]
   ```

   **Совет:** Используйте диапазоны для массового добавления ID. Например, `"100-200"` добавит 101 коктейль в очередь!

3. **Запустите парсинг**:
   ```bash
   npm run parse:inshaker
   ```

4. **Проверьте результат**:
   - Скрипт выведет статистику парсинга
   - Успешные ID → `parsed_ids.json` + `inshaker_recipes.json`
   - Ошибки → `failed_ids.json`
   - Очередь `to_parse.json` очищена

5. **Перезапустите бота** (если нужно):
   ```bash
   # Локально
   npm run dev

   # На сервере
   pm2 restart cocktail-bot
   ```

### Пример вывода парсинга

**Входные данные** (`data/to_parse.json`):
```json
[
  "160-165",
  200,
  "https://ru.inshaker.com/cocktails/201"
]
```

**Вывод скрипта:**
```
🚀 Запуск парсера Inshaker с системой таблиц

📚 Загружено 93 рецептов из базы
📊 Статус таблиц:
   ✅ Успешно спарсено: 93 ID
   ❌ Не удалось спарсить: 0 ID
   ⏳ В очереди на парсинг: 3 ID

🍸 Начинаем парсинг 8 новых коктейлей:

Парсинг коктейля ID 160...
✅ Успешно: Арбузный коблер 317 (8 ингредиентов)
Парсинг коктейля ID 161...
✅ Успешно: Арбузный слинг (5 ингредиентов)
Парсинг коктейля ID 162...
✅ Успешно: Арбузный тай слинг (7 ингредиентов)
Парсинг коктейля ID 163...
✅ Успешно: Арбузный эль (5 ингредиентов)
Парсинг коктейля ID 164...
✅ Успешно: Б-52 (3 ингредиентов)
Парсинг коктейля ID 165...
✅ Успешно: Баба Таня (8 ингредиентов)
Парсинг коктейля ID 200...
✅ Успешно: Бока чика (11 ингредиентов)
Парсинг коктейля ID 201...
✅ Успешно: Большое яблоко (9 ингредиентов)

✨ Парсинг завершён!

📊 Результаты:
   ✅ Успешно спарсено: 8 рецептов
   ❌ Ошибки парсинга: 0 ID
   ⏭️  Пропущено: 0 ID

📁 База рецептов обновлена: data/inshaker_recipes.json
   - Всего рецептов: 101
   - Алкогольных: 101
   - Безалкогольных: 0

📋 Таблицы обновлены:
   - data/parsed_ids.json (101 ID)
   - data/failed_ids.json (0 ID)
   - data/to_parse.json (очищена)
```

💡 **Обратите внимание:** Диапазон `"160-165"` автоматически расширился в 6 ID!

### Преимущества системы таблиц

- ✅ Не парсит повторно существующие рецепты
- ✅ Не тратит запросы на несуществующие ID (404)
- ✅ Поддерживает ID, полные ссылки и **диапазоны** (`"100-200"`)
- ✅ Сохраняет информацию об ошибках с метаданными
- ✅ Автоматическая очистка очереди после обработки
- ✅ Задержка 1 секунда между запросами (не перегружает сервер)
- ✅ Массовое добавление рецептов через диапазоны (например, `"1-1000"` для парсинга 1000 ID)

### Структура рецепта

Каждый рецепт в `data/inshaker_recipes.json` имеет формат:
```json
{
  "id": 724,
  "name": "Порнстар Мартини",
  "image": "/uploads/cocktail/promo/724/...",
  "category": "Коктейли с водкой",
  "tags": ["Сладкий", "Фруктовый"],
  "glass": "Бокал для мартини",
  "ingredients": [
    { "amount": "50 мл", "name": "Водка" },
    { "amount": "20 мл", "name": "Ликер Пассоа" }
  ],
  "instructions": [
    "Разрежь маракуйю пополам...",
    "Налей в шейкер..."
  ],
  "rating": 4.5,
  "alcoholic": true,
  "source": "inshaker",
  "parsedAt": "2026-02-14T12:00:00.000Z"
}
```

## Деплой сайта (Яндекс Object Storage)

Бакет: `alcoberry.ru`, профиль AWS CLI: `yandex`

```bash
# Пересобрать и загрузить
npm run web:build
aws s3 sync web/out/ s3://alcoberry.ru/ \
  --endpoint-url https://storage.yandexcloud.net \
  --profile yandex
```

**Инфраструктура сайта:**
- Хостинг: Яндекс Object Storage (бакет `alcoberry.ru`)
- DNS: Яндекс Cloud DNS (NS: `ns1/ns2.yandexcloud.net`)
- HTTPS: Яндекс Certificate Manager (Let's Encrypt, автопродление)
- Аналитика: Яндекс.Метрика ID `106933420`

---

## Деплой бота на сервер

Для обновления бота на продакшн-сервере (Яндекс.Облако):
```bash
# 1. Подключиться к серверу
ssh kudinow@158.160.150.22

# 2. Перейти в директорию проекта
cd ~/cocktail-bot

# 3. Получить последние изменения
git pull origin main

# 4. Установить зависимости
npm install

# 5. Собрать проект
npm run build

# 6. Перезапустить через PM2
pm2 restart cocktail-bot

# 7. Проверить логи
pm2 logs cocktail-bot --lines 50
```

**PM2 команды:**
- `pm2 status` — статус всех процессов
- `pm2 restart cocktail-bot` — перезапуск бота
- `pm2 logs cocktail-bot` — просмотр логов в реальном времени
- `pm2 save` — сохранить текущую конфигурацию
- `pm2 startup` — автозапуск при перезагрузке сервера
